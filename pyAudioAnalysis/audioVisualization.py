...
def generateColorMap():
    ...
    for i in range(Map.shape[0]):
        ...
        if sys.version_info > (3, 0):
            ...
            stringColors.append(struct.pack("BBB", *rgb).encode("hex"))  # python2
...
def text_list_to_colors(names):
    ...
    for i in range(len(names)):
        for j in range(len(names)):
            Dnames[i, j] = 1 - 2.0 * levenshtein(names[i], names[j]) / float(
                len(names[i] + names[j])
            )
...
def visualizeFeaturesFolder(folder, dimReductionMethod, priorKnowledge="none"):
    ...
    if dimReductionMethod == "pca":
        ...
        names_category_toviz = [
            ntpath.basename(w).replace(".wav", "").split(" --- ")[0] for w in wav_files
        ]
        ...
        all_mt_feat, Ys, wav_files = aF.directory_feature_extraction_no_avg(
