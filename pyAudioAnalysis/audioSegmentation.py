...
def plot_segmentation_results(
    flags_ind, flags_ind_gt, class_names, mt_step, evaluate_only=False
):
    """
    ...
    if min_len > 0:
        accuracy = np.sum(flags_ind[0:min_len] == flags_ind_gt[0:min_len]) / float(
            min_len
        )
    else:
        accuracy = -1
    ...
    if not evaluate_only:
        ...
        if flags_ind_gt.shape[0] > 0:
            ax1.plot(
                np.array(range(len(flags_ind_gt))) * mt_step + mt_step / 2.0,
                flags_ind_gt + 0.05,
                "--r",
            )
...
def evaluate_speaker_diarization(labels, labels_gt):
    ...
    for i in range(min_len):
        contigency_matrix[
            int(np.nonzero(unique_flags == labels[i])[0]),
            int(np.nonzero(unique_flags_gt == labels_gt[i])[0]),
        ] += 1.0
...
def train_hmm_compute_statistics(features, labels):
    ...
    if features.shape[1] < labels.shape[0]:
        print(
            "trainHMM warning: number of short-term feature vectors "
    ...
    for i in range(n_comps):
        means[i, :] = np.array(
            features[:, np.nonzero(labels == unique_labels[i])[0]].mean(axis=1)
        )
    ...
    for i in range(n_comps):
        ...
        cov[i, :] = np.std(
            features[:, np.nonzero(labels == unique_labels[i])[0]], axis=1
        )
...
def train_hmm_from_file(wav_file, gt_file, hmm_model_name, mid_window, mid_step):
    ...
    class_priors, transumation_matrix, means, cov = train_hmm_compute_statistics(
        features, flags
    )
...
def train_hmm_from_directory(folder_path, hmm_model_name, mid_window, mid_step):
    ...
    for i, f in enumerate(glob.glob(folder_path + os.sep + "*.wav")):
        ...
        if os.path.isfile(gt_file):
            seg_start, seg_end, seg_labs = read_segmentation_gt(gt_file)
            flags, class_names = segments_to_labels(
...
def hmm_segmentation(audio_file, hmm_model_name, plot_results=False, gt_file=""):
    ...
    labels_gt, class_names_gt, accuracy, cm = load_ground_truth(
        gt_file, labels, class_names, mid_step, plot_results
    )
...
def mid_term_file_classification(
    ...
    if compute_beat:
        print(
            "Model " + model_name + " contains long-term music features "
    ...
    mt_feats, _, _ = mtf.mid_feature_extraction(
        signal,
        sampling_rate,
    ...
    for col_index in range(mt_feats.shape[1]):
        ...
        # classify vector:
        label_predicted, posterior = at.classifier_wrapper(
            classifier, model_type, feature_vector
        )
...
def load_ground_truth(gt_file, labels, class_names, mid_step, plot_results):
    ...
    if os.path.isfile(gt_file):
        ...
        accuracy = plot_segmentation_results(
            labels_new, labels_gt, class_names_gt, mid_step, not plot_results
        )
        if accuracy >= 0:
...
def evaluate_segmentation_classification_dir(dir_name, model_name, method_name):
    ...
    for index, wav_file in enumerate(glob.glob(dir_name + os.sep + "*.wav")):
        ...
        if method_name.lower() in [
            ...
            flags_ind, class_names, accuracy, cm_temp = mid_term_file_classification(
                wav_file, model_name, method_name, False, gt_file
            )
        else:
            flags_ind, class_names, accuracy, cm_temp = hmm_segmentation(
                wav_file, model_name, False, gt_file
            )
        if accuracy > 0:
            if not index:
    ...
    if len(cm_total.shape) > 1:
        ...
        print(
            "Median Accuracy: {0:.1f}".format(100.0 * np.median(np.array(accuracies)))
        )
...
def silence_removal(
    signal, sampling_rate, st_win, st_step, smooth_window=0.5, weight=0.5, plot=False
):
    """
    ...
    signal = audioBasicIO.stereo_to_mono(signal)
    st_feats, _ = stf.feature_extraction(
        signal, sampling_rate, st_win * sampling_rate, st_step * sampling_rate
    )
    ...
    threshold = np.mean((1 - weight) * prog_on_set_sort[0:nt]) + weight * np.mean(
        prog_on_set_sort[-nt::]
    )
    ...
    if plot:
        time_x = np.arange(
            0, signal.shape[0] / float(sampling_rate), 1.0 / sampling_rate
        )
...
def speaker_diarization(
    ...
    classifier_all, mean_all, std_all, class_names_all, _, _, _, _, _ = at.load_model(
        os.path.join(base_dir, "svm_rbf_speaker_10")
    )
    classifier_fm, mean_fm, std_fm, class_names_fm, _, _, _, _, _ = at.load_model(
        os.path.join(base_dir, "svm_rbf_speaker_male_female")
    )

    mid_feats, st_feats, a = mtf.mid_feature_extraction(
        signal,
        sampling_rate,
        mid_window * sampling_rate,
        mid_step * sampling_rate,
    ...
    if lda_dim > 0:
        ...
        mt_feats_to_red_2 = np.zeros(
            (
                mt_feats_to_red.shape[0] + len(class_names_all) + len(class_names_fm),
        ...
        for index in range(mt_feats_to_red.shape[1]):
            ...
            _, p1 = at.classifier_wrapper(classifier_all, "svm_rbf", feature_norm_all)
            _, p2 = at.classifier_wrapper(classifier_fm, "svm_rbf", feature_norm_fm)
            mt_feats_to_red_2[0 : mt_feats_to_red.shape[0], index] = mt_feats_to_red[
                :, index
            ]
        ...
        for c in range(speakers):
            ...
            if clust_per_cent < 0.020:
                ...
                for c2 in range(speakers):
                    ...
                    if c2 != c:
                        clust_per_cent_2 = np.nonzero(cls == c2)[0].shape[0] / float(
                            len(cls)
                        )
                        mid_features_temp = mid_feats_norm[cls == c2, :]
                        dist = distance.cdist(mt_feats_norm_temp, mid_features_temp)
                        sil_temp.append(
                            np.mean(dist) * (clust_per_cent + clust_per_cent_2) / 2.0
                        )
    ...
    if lda_dim <= 0:
        for index in range(1):
            # hmm training
            start_prob, transmat, means, cov = train_hmm_compute_statistics(
                mt_feats_norm_or.T, cls
            )
    ...
    if os.path.isfile(gt_file):
        seg_start, seg_end, seg_labs = read_segmentation_gt(gt_file)
        flags_gt, class_names_gt = segments_to_labels(
            seg_start, seg_end, seg_labs, mid_step
        )
...
def speaker_diarization_evaluation(folder_name, lda_dimensions):
    ...
    for dim in lda_dimensions:
        ...
        for i, wav_file in enumerate(wav_files):
            speaker_diarization(
                wav_file, num_speakers[i], 2.0, 0.2, 0.05, dim, plot_res=False
            )


def music_thumbnailing(
    signal,
    sampling_rate,
    short_window=1.0,
    short_step=0.5,
    ...
    # feature extraction:
    st_feats, _ = stf.feature_extraction(
        signal, sampling_rate, sampling_rate * short_window, sampling_rate * short_step
    )
    ...
    while i2 - i1 < m_filter:
        if (
            i1 <= 0
            or j1 <= 0
            or i2 >= sim_matrix.shape[0] - 2
            or j2 >= sim_matrix.shape[1] - 2
        ):
            break
