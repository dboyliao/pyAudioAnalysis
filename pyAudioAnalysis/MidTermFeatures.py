...
def beat_extraction(short_features, window_size, plot=False):
    ...
    for ii, i in enumerate(selected_features):
        ...
        dif_threshold = (
            2.0 * (np.abs(short_features[i, 0:-1] - short_features[i, 1::])).mean()
        )
        if dif_threshold <= 0:
        ...
        histogram_times, histogram_edges = np.histogram(
            position_diffs, np.arange(0.5, max_beat_time + 1.5)
        )
...
def mid_feature_extraction(
    ...
    short_features, short_feature_names = ShortTermFeatures.feature_extraction(
...
def directory_feature_extraction(
    folder_path, mid_window, mid_step, short_window, short_step, compute_beat=True
):
    """
    ...
    for i, file_path in enumerate(wav_file_list):
        ...
        if compute_beat:
            mid_features, short_features, mid_feature_names = mid_feature_extraction(
                signal,
                sampling_rate,
            ...
            mid_features, _, mid_feature_names = mid_feature_extraction(
                signal,
                sampling_rate,
                round(mid_window * sampling_rate),
                round(mid_step * sampling_rate),
...
def multiple_directory_feature_extraction(
    path_list, mid_window, mid_step, short_window, short_step, compute_beat=False
):
    """
    ...
    for i, d in enumerate(path_list):
        f, fn, feature_names = directory_feature_extraction(
            d, mid_window, mid_step, short_window, short_step, compute_beat=compute_beat
        )
        if f.shape[0] > 0:
...
def directory_feature_extraction_no_avg(
    ...
    for i, file_path in enumerate(wav_file_list):
        ...
        if sampling_rate == 0:
            continue
        signal = audioBasicIO.stereo_to_mono(signal)
        mid_feature_vector, _, _ = mid_feature_extraction(
            signal,
            sampling_rate,
            round(mid_window * sampling_rate),
            round(mid_step * sampling_rate),
...
def mid_feature_extraction_to_file(
    file_path,
    mid_window,
    mid_step,
    short_window,
...
def mid_feature_extraction_file_dir(
    folder_path,
    mid_window,
    mid_step,
    short_window,
