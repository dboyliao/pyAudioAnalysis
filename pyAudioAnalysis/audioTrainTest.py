...
class Knn:
    ...
    def classify(self, test_sample):
        ...
        y_dist = (
            distance.cdist(
                self.features, test_sample.reshape(1, test_sample.shape[0]), "euclidean"
            )
        ).T
        i_sort = np.argsort(y_dist)
        P = np.zeros((n_classes,))
        for i in range(n_classes):
            P[i] = np.nonzero(self.labels[i_sort[0][0 : self.neighbors]] == i)[0].shape[
                0
...
def classifier_wrapper(classifier, classifier_type, test_sample):
    ...
    if classifier_type == "knn":
        ...
        classifier_type == "svm"
        or classifier_type == "randomforest"
        or classifier_type == "gradientboosting"
        or classifier_type == "extratrees"
        or classifier_type == "svm_rbf"
    ):
...
def extract_features_and_train(
    ...
    features, class_names, file_names = aF.multiple_directory_feature_extraction(
        paths, mid_window, mid_step, short_window, short_step, compute_beat=compute_beat
    )
...
def feature_extraction_train_regression(
    folder_name,
    mid_window,
    mid_step,
    short_window,
    ...
    """
    # STEP A: Feature Extraction:
    features, _, filenames = aF.multiple_directory_feature_extraction(
        [folder_name],
        mid_window,
        mid_step,
        short_window,
            ...
            for row in csv_reader:
                if len(row) == 2:
                    ...
                    if row[0] in filenames:
                        ...
                        print(
                            "Warning: {} not found " "in list of files.".format(row[0])
                        )
                else:
    ...
    for iRegression, r in enumerate(regression_names):
        ...
        if model_type == "svm":
            classifier, _ = train_svm_regression(
                features_norm, regression_labels[iRegression], bestParam
            )
        if model_type == "svm_rbf":
            classifier, _ = train_svm_regression(
                features_norm, regression_labels[iRegression], bestParam, kernel="rbf"
            )
...
def evaluate_regression(features, labels, n_exp, method_name, params):
    ...
    for Ci, C in enumerate(params):  # for each param value
        ...
        for e in range(n_exp):  # for each cross-validation iteration:
            ...
            f_test = [
                features_norm[randperm[i + n_train]] for i in range(n_samples - n_train)
            ]
            ...
            if method_name == "svm":
                ...
                classifier, train_err = train_svm_regression(
                    f_train, l_train, C, kernel="rbf"
                )
            elif method_name == "randomforest":
                classifier, train_err = train_random_forest_regression(
                    f_train, l_train, C
                )
            error_test = []
            error_test_baseline = []
            for itest, fTest in enumerate(f_test):
                ...
                error_test_baseline.append(
                    (Rbaseline - l_test[itest]) * (Rbaseline - l_test[itest])
                )
    ...
    for i in range(len(errors_all)):
        print(
            "{0:.4f}\t\t{1:.2f}\t\t{2:.2f}\t\t{3:.2f}".format(
                params[i], errors_all[i], er_train_all[i], er_base_all[i]
            ),
            end="",
        )
        if i == best_ind:
...
def evaluate_model_for_folders(
    input_test_folders, model_name, model_type, positive_class, plot=True
):
    """
    ...
    pre, rec, thr_prre = sklearn.metrics.precision_recall_curve(
    ...
    if plot:
        ...
        figs.append_trace(
            go.Scatter(x=thr_prre, y=pre, name="Precision", marker=mark_prop1), 2, 1
        )
        figs.append_trace(
            go.Scatter(x=thr_prre, y=rec, name="Recall", marker=mark_prop2), 2, 1
        )
...
def file_classification(input_file, model_name, model_type):
    ...
    # feature extraction:
    mid_features, s, _ = aF.mid_feature_extraction(
        signal,
        sampling_rate,
